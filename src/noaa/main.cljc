(ns noaa.main
  "Kotodama WASM entrypoint for the NOAA clean-room actor (L4) — Clojure port.
  ...
  facts (`noaa.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Filing"       :plural "filings"       :id-prefix "noaa_fil"
    :fields [:filerId :form :period :status]      :required [:form :period]
    :coerce {}                       :refs {}}
   {:entity "Entity"       :plural "entities"      :id-prefix "noaa_ent"
    :fields [:name :ein :jurisdiction]            :required [:name :ein]
    :coerce {}                       :refs {}}
   {:entity "Dataset"      :plural "datasets"      :id-prefix "noaa_dat"
    :fields [:name :agency :updatedAt]            :required [:name :agency]
    :coerce {}                       :refs {}}
   {:entity "Application"  :plural "applications"  :id-prefix "noaa_app"
    :fields [:applicantId :program :status]       :required [:program :status]
    :coerce {}                       :refs {}}
   {:entity "PublicRecord" :plural "publicrecords" :id-prefix "noaa_pub"
    :fields [:recordType :jurisdiction :recordedAt] :required [:recordType :jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Notice"       :plural "notices"       :id-prefix "noaa_not"
...
(defn emit-facts
  "EAVT facts for one record: {\"noaa.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "noaa-compat" :tier tier :entities entities} 200])
